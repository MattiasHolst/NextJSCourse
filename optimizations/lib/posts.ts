...
function initDb() {
  ...
  const response = stmt.get() as CountResult;
...
export async function getPosts(maxNumber?: number) {
  ...
  return maxNumber
    ? (stmt.all(maxNumber) as GetPostType[])
    : (stmt.all() as GetPostType[]);
}
...
export async function updatePostLikeStatus(postId: number, userId: number) {
  ...
  const response = stmt.get(userId, postId) as CountResult;

  const isLiked = response.count === 0;
